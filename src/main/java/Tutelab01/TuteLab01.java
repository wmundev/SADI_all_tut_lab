...
public class TuteLab01 {
    ...
    // I do the main work in the constructor since this is basically a single
    // use class
    ...
    private TuteLab01() {
        ...
        for (int i = 0; i < NUM_INPUTS; i++) {
            // we put the input method in a separate class since it is
            // general purpose/reusable
            volts[i] = ScannerUtils.nextDoubleValidated("Enter Voltage (V) in" +
                    " volts: ");
            amps[i] = ScannerUtils.nextDoubleValidated("Enter Current (I) in " +
                    "amperes: ");
            resistance[i] = volts[i] / amps[i];
            System.out.printf("Resistance (R) is %.2f ohms\n\n", resistance[i]);
        }

        // print the summary info (note the code reuse by passing the
        // relevant array)
    ...
    // these could go in an external class as static methods since they are
    // reusable but can be private for this example
    ...
    private double calcArrayMax(double[] array) {
        // the first number will be more (unless it is already Double
        // .MIN_VALUE) to get us started
    ...
    private double calcArrayMin(double[] array) {
        // the first number will be less (unless it is already Double
        // .MAX_VALUE) to get us started
    ...
    private void printSummary(String label, double[] array) {
        // no unnecessary variables just call the function and display its
        // output via printf!
        System.out.printf("%s .. Avg=%.2f, Max=%.2f, Min=%.2f\n", label,
                calcArrayAvg(array), calcArrayMax(array),
                calcArrayMin(array));
    }
}
